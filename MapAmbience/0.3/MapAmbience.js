...
const Transition = (() => {
    ...
    // Constants
	const version = '0.3'; //eslint-disable-line no-unused-vars
	...
	let jbFadeResetCache = {};
	let dlFadeReset;
	let transparifyResetCache = {};
    ...
    transparifyImpl = (operation, incomingId, increment, ms, reset) => {
        ...
        return new Promise(resolve => {
            ...
            if(reset) {
                if(transparifyResetCache[incomingId]) {
                    // Already processed, but we still have to resolve
                    resolve("AlreadyDone");
                }
                else {
                    wrapper.reset(operation);
                    transparifyResetCache[incomingId] = incomingId;
    ...
    transparifySequence = (msg, args) => {
        ...
        _.each(idArray, function(idObj) {
            clog("- idObj: " + JSON.stringify(idObj));
        });
                    ...
                    _.each(idObj.ids, function(id) {
                        ...
                        if(id.toLowerCase().startsWith("wrap-macro:")) {
                            ...
                            if(reset) {
                                ...
                                // value, so we just set it to 1.0 if its 0.0 and vice-versa
                                if(+params[2] === 1) {
                                    params[2] = "0.0";
                                }
                                else if(+params[2] === 0) {
                            ...
                            clog("- Storing promise for transparify, operation: " + operation + ", id: " + id);
                        ...
                        if(idObj.delay) {
                            ...
                            await sleep(reset ? 0 : idObj.delay).then(function() {
                                ...
                                if(thenCallbacks[callbackCounter]) {
                                    ...
                                    clog("- All transparify operations complete");
                                    jbFadeResetCache = {};
                                    transparifyResetCache = {};
                                    dlFadeReset = false;
                                }
                            });
                        }
    ...
    fadeDaylight = (msg, args, reset) => {
        ...
        fadeDaylightImpl(delta, target, ms, reset);
    }
    ...
    fadeJukebox = (msg, args, reset) => {
        ...
        if(reset) {
            if(jbFadeResetCache[p.trackName]) {
                clog("- Ignoring jukebox fade for: " + p.trackName + " to " + p.startVolume + " as already processed");
                return;
            }
            else {
                clog("- Resetting jukebox fade for: " + p.trackName + " to " + p.startVolume);
                // Set delay to 10ms so we reset quickly
                jbTrack.set("volume", p.startVolume);
                jbTrack.set("playing", false);
                ...
                jbFadeResetCache[p.trackName] = jbTrack;
            }
            return;
        }
        ...
        const interval = runSetInterval(function() {
            ...
            if(operations[operation].complete(volume)) {
                ...
                clog("- fadeJukebox complete for: " + p.trackName);
            }
            else {
