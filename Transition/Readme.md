...
Here is a video showing my opening page just to give a sense of what the script can do:
...
Fade in Shape C and Text AA simultaneously THEN<br>
Wait 5 seconds THEN<br>
Fade out Daylight THEN<br>
Fade out Music
...
- Jukebox: Increment 2, interval 800ms (range 0 to 100)
- Daylight: Increment 0.025, interval 50ms (range 0.0 to 1.0 range))

**NOTE 3**: The script automatically ‘absolutes’ increment values and sets the sign according to the operation.

**WARNING**: Beware of setting milliseconds too small, I found that around 50ms or less the transitions could get a bit laggy. Again maybe browser,/computer dependent.

**TIP**: If you are fading in, then you need to start from transparent for your selected objects. As you cannot use ‘transparent’ (as the script would never know what your target colour would be) you should EITHER set your colours manually appended with ’00’ to force transparency OR select the items and add reset as the last argument to Transition. I went through MANY iterations of my opening sequence to get the timings etc. correct, so for most uses of Transition you can append with ‘reset’ to get back to your starting state.

When building your command it is easiest to do it in a macro and add: ?{Reset||reset} to allow easy run and reset cycles. Caveats to this are:

- Resetting daylight fades will set back to 100% brightness
- Resetting jukebox fades sets track volume to 0. Unfortunately you cannot reset a track to its starting play point as that is not exposed by the API
- Macros wrapped by Transition obviously cannot be reset

**NOTE 4**: Jukebox Fade and Daylight Fade and Wrapped Macros are ASYNCHRONOUS, they will run while other sequences are running. For me this made sense with the use cases I had in mind and it is much easier to just multiply the increments by the ms delays and add an equivalent pause after an operation to force synchronicity, rather than pollute the ‘keyword space’ with synchronous versions, e.g.:
...
Transition has a number of ‘convenience’ actions for ‘quick and dirty’ effects, these are:
###
...
###
####  !transition fade-jukebox <track> <start volume or ‘current’> <end volume> [increment, default: 2] [milliseconds, default: 800] [reset]
...
Should any intervals get out of control, e.g. get into an infinite loop (which should not be possible!) then this command will kill any running intervals. Use the debug-on command to check for an infinite loop.
...
So we can chain together series of operations using a number of elements, the =n notation signifies defaults where the element can be omitted:
...
**#Comment**: As it can get a bit confusing with lots of IDs any term starting with # is ignored as a comment

Number: Any number between sequence items is used as a delay in milliseconds

-Naaaaaaa or -Naaaaaa|-Nbbbbbb: Shape or path IDs for transparency transitions. Each ID tuple can take the form **_ID:[in/out]:[Increment=4]:[Delay=80]_** and multiple tuples can be pipe-delimited together.<br> 

Note that the fade in/out variable defaults as follows: Shapes will fade **out** to transparent, text will fade **in** from transparent. That is why in the example above only the IDs are shown with no colon-delimited extra information. Joining these ID tuples with a pipe symbol means they will execute simultaneously.

So this: **-Nq4HzffCq5YTIANqhEs|-Nq4ILn0YaV8Z4GA6R6J**<br>
